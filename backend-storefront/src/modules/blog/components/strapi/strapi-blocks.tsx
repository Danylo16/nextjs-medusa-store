...
function unwrapMedia(input: any) {
  // Supports direct image attributes or { data: { attributes } } shape.
...
function buildSrcSetFromImage(imageInput: any): {
  src?: string
  srcSet?: string
  width?: number
  height?: number
  ...
  const image = unwrapMedia(imageInput)
  if (!image) return {}
...
const blocks = {
  ...
  link: (props: WithChildren & { url?: string }) => {
    ...
    const cls =
      "underline underline-offset-4 decoration-black/20 hover:decoration-black/35 transition"
...
const modifiers = {
  bold: (props: WithChildren) => <strong className="font-semibold">{props.children}</strong>,
  italic: (props: WithChildren) => <em className="italic">{props.children}</em>,
  underline: (props: WithChildren) => (
    <span className="underline underline-offset-4">{props.children}</span>
  ),
  strikethrough: (props: WithChildren) => <span className="line-through">{props.children}</span>,
  code: (props: WithChildren) => (
    <code className="rounded-md bg-black/5 px-1.5 py-0.5 font-mono text-[13px] text-foreground">
